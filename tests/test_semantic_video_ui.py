...
def test_semantic_controller_confirms_exact_cost_and_polls_progress():
    ...
    assert "for (const root of roots)" in source
    assert "Promise.all(roots.map" not in source
    ...
    assert template_source.count(
        'hx-headers=\'{"X-Delivery-Navigation-Owner": "client"}\''
    ) == 2
