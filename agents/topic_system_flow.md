...
- Selection only reuses `audit_status='pass'` coverage and only if `use_count == 0`; once a script has been used for a post it is one-and-done, and future reuse must come from a fresh variant in the same family.
...
- Used scripts are never selected again for another post; family reuse stays at the topic-family level, not the exact script level.
