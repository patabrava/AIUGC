...
- `static/js/batches/detail.js` owns calendar state and derived behavior inside `batchPublishComponent`: selected item, Monday-normalized seven-day navigation, social and blog events, precise time editing, readiness, conflicts, and the unchanged atomic arm request.
...
- Layout: shared controls in the header; queue, calendar, and inspector form the desktop workspace; sections stack on smaller screens; the seven calendar columns compress within their surface so no day becomes a hidden horizontal column.
- Density: Monday through Sunday remain visible at every width. Narrow event cards reduce their visible label detail while retaining their full accessible name and selected-item precision controls.
...
- A fixed Monday-through-Sunday layout preserves every valid weekday and weekend schedule without a mode switch.
...
- The calendar uses seven fluid day columns at narrow widths. The page and calendar surfaces avoid horizontal overflow, while full event details remain available in the selected-item inspector.
...
- Exercised social date/time, blog datetime, destination change, paired event rendering, selected event, Friday click placement, item validation, readiness transition, fixed seven-day rendering, desktop fit, mobile structure, and console errors.
