...
def _ensure_google_adc_env() -> None:
    ...
    adc_path = resolve_google_application_credentials_path(settings)
    if adc_path and not os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = adc_path
    project_id = (settings.vertex_ai_project_id or "").strip()
    if project_id:
...
def _acquire_poller_lock() -> bool:
    """Ensure only one poller process owns leases at a time."""
    global _VIDEO_POLLER_LOCK_HANDLE
    try:
        import fcntl
    except ImportError:  # pragma: no cover - Windows fallback
        logger.warning("video_poller_lock_unavailable", reason="fcntl_missing")
        return True

    handle = open(VIDEO_POLLER_LOCK_PATH, "w")
    try:
        fcntl.flock(handle.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    except OSError:
        handle.close()
        logger.warning(
            "video_poller_lock_unavailable",
            lock_path=VIDEO_POLLER_LOCK_PATH,
            message="Another video poller process is already running",
        )
        return False

    _VIDEO_POLLER_LOCK_HANDLE = handle

    def _release_lock() -> None:
        global _VIDEO_POLLER_LOCK_HANDLE
        if _VIDEO_POLLER_LOCK_HANDLE is None:
            return
        try:
            fcntl.flock(_VIDEO_POLLER_LOCK_HANDLE.fileno(), fcntl.LOCK_UN)
        finally:
            _VIDEO_POLLER_LOCK_HANDLE.close()
            _VIDEO_POLLER_LOCK_HANDLE = None
...
def _clear_expired_poller_leases() -> None:
    """Remove expired lease metadata left behind by crashed or restarted workers."""
    supabase = get_supabase().client
    now = _utc_now()
    statuses = get_pollable_video_statuses()
    response = (
        supabase.table("posts")
        .select("id,updated_at,video_metadata")
    ...
    for post in posts:
        ...
        if not lease_owner or not expires_at or expires_at > now:
            continue
        metadata.pop(VIDEO_POLL_LEASE_OWNER_KEY, None)
        metadata.pop(VIDEO_POLL_LEASE_ACQUIRED_KEY, None)
        metadata.pop(VIDEO_POLL_LEASE_EXPIRES_KEY, None)
        metadata["last_poll_recovery"] = "startup_expired_lease_cleanup"
        supabase.table("posts").update({
            "video_metadata": metadata,
        }).eq("id", post["id"]).execute()
        logger.info(
            "video_poller_expired_lease_cleared",
            post_id=post["id"],
...
def _clear_terminal_polling_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in (
        VIDEO_POLL_LEASE_OWNER_KEY,
        VIDEO_POLL_LEASE_ACQUIRED_KEY,
        VIDEO_POLL_LEASE_EXPIRES_KEY,
        "last_polled_by",
        "last_polled_at",
        "error",
        "error_type",
...
def process_video_operation(post: Dict[str, Any]):
    ...
    if not operation_id or not provider:
        logger.warning(
            ...
            has_metadata_operation_id=bool(metadata.get("operation_id")),
            has_metadata_provider=bool(metadata.get("provider")),
        )
        return
    ...
    post = claimed_post
    operation_id = post.get("video_operation_id") or (post.get("video_metadata") or {}).get("operation_id")
    provider = post.get("video_provider") or (post.get("video_metadata") or {}).get("provider")
...
def _store_completed_video(
    *,
    ...
    if isinstance(processed_source, str) and processed_source.startswith(("gs://", "data:")):
        processed_source = _decode_vertex_video_uri(processed_source)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("video_poller_startup_recovery_failed", error=str(exc))
