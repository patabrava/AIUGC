...
def _build_fallback_research_dossier_text(*, seed_topic: str, post_type: str) -> str:
    return "\n".join(
        [
            ...
            "",
            f"Dieses Dossier ersetzt den gemini Deep-Research-Output für {post_type}-Content, wenn der Provider beim Polling ausfällt.",
            ...
            f"- Wie Nutzerinnen und Nutzer {seed_topic} ohne Hürden vorbereiten und abstimmen können.",
            "## Digitale Zugangswege für kleine Praxen ohne Barrieren",
            f"- Formulare, Portale und Rückrufwege müssen im Alltag leicht erreichbar sein.",
            "## Rückruf, Erreichbarkeit und klare Abläufe bei Terminen",
            ...
            "- Zeitdruck und unklare Zuständigkeiten können Entscheidungen verzögern.",
            "- Fehlende Vorbereitung führt oft zu Rückfragen und vermeidbaren Fehlern.",
            "- Unterschiedliche Ausgangslagen brauchen saubere Einordnung statt Pauschallösungen.",
            "",
            "## Faktenlage für die praktische Terminplanung",
            f"- {seed_topic} bleibt das zentrale Suchthema dieses Laufs.",
            "- Das Thema wird für den deutschen Sprachraum lokal normalisiert.",
            "- Der Flow erzeugt daraus kanonische 8/16/32 Varianten für die Topic Bank.",
        ]
    )
