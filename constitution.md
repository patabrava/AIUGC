...
- **Rate limiting.** Implement per-user rate limits on expensive operations (video generation, publishing).
- **Audit logs.** Record who did what when for state transitions and publishes.

### DON'T: Expose Vulnerabilities
- **No SQL injection.** Use Supabase SDK parameterized queries; never string-concatenate SQL.
- **No XSS.** Jinja2 auto-escapes by default; do not use `|safe` without sanitization.
- **No secret leakage.** Redact tokens/keys in logs and error messages.

---

## XV. PROGRESSIVE ENHANCEMENT

### DO: Build Resilient UI
- **Server-side rendering.** Jinja2 templates render full HTML; app works without JS.
- **htmx for interactivity.** Use `hx-get`, `hx-post`, `hx-swap` for AJAX; fallback to full page load.
- **Alpine.js for local state.** Use for UI-only state (modals, dropdowns); do not duplicate server state.
- **Tailwind for styling.** Use Material Design 3 tokens; keep utility classes semantic.

### DON'T: Require JavaScript for Core Flows
- **No SPA assumptions.** Do not rely on client-side routing or state management for critical paths.
- **No broken fallbacks.** If JS disabled, forms must still POST and pages must still navigate.

---

## XVI. DEPLOYMENT & OPS

### DO: Ensure Smooth Operations
- **Health checks.** `/health` endpoint returns 200 if DB connected and jobs running.
- **Graceful shutdown.** Handle SIGTERM; finish in-flight requests before exit.
- **Migrations first.** Run DB migrations before deploying new code; ensure backward compatibility.
- **Rollback plan.** Tag releases; document rollback procedure in `DEPLOYMENT.md`.

### DON'T: Deploy Broken Code
- **No untested deploys.** All testscripts must pass before pushing to production.
- **No config drift.** Verify env vars match between staging and production.
- **No silent failures.** Monitor error rates and alert on anomalies.

---

## XVII. COLLABORATION WITH HUMAN-AS-EYE

### DO: Provide Clear Instructions
- **Runnable commands.** Specify exact shell commands with working directory.
- **Observable checkpoints.** Tell human what to see at each step (e.g., "Verify batch state is S2_SEEDED").
- **Artifact requests.** List specific files/logs/screenshots to capture with paths and timestamps.
- **Minimal data requests.** Ask only for high-signal evidence; avoid fishing expeditions.

### DON'T: Assume or Guess
- **No vague instructions.** "Check if it works" is insufficient; specify exact success criteria.
- **No background promises.** Deliver concrete, runnable artifacts now; no "I'll fix it later."
- **No speculation.** If root cause unclear, request targeted evidence, not broad dumps.

---

## XVIII. EVOLUTION & REFACTORING

### DO: Improve Incrementally
- **Inline then extract.** Move logic next to caller; extract only after "rule of three."
- **Contract-first refactoring.** Stabilize API schemas before changing internals.
- **Strangler fig pattern.** Introduce new slice alongside old; route traffic gradually.
- **Regression protection.** Ensure all testscripts pass after refactor; add new checks if needed.

### DON'T: Break Working Code
- **No big-bang rewrites.** Refactor in small, verifiable steps with testscript coverage.
- **No contract changes without canon.** API/schema changes require canon amendment first.
- **No orphaned code.** Remove dead code immediately; do not leave commented-out blocks.
...
1. ✅ **Canon conformance:** Feature matches `CANON.md` specification exactly.
2. ✅ **Validated boundaries:** All inputs/outputs have Pydantic schemas and runtime checks.
3. ✅ **Testscript green:** Phase-specific whole-app testscript passes.
4. ✅ **Regression clean:** All prior phase testscripts still pass.
5. ✅ **Observable:** Structured logs with correlation IDs at boundaries.
6. ✅ **Deterministic:** Pinned versions, seeded randomness, idempotent operations.
7. ✅ **Documented:** README/CANON updated if new surfaces introduced.
8. ✅ **Secure:** No hardcoded secrets, inputs validated, auth enforced.

**If any criterion fails, work is not done. Fix before proceeding.**

---

## XX. ENFORCEMENT

- **LLM assistants must cite this Constitution when making decisions.**
- **Deviations require explicit canon amendment with version bump.**
- **Human-as-EYE may reject work that violates these rules.**
- **All code reviews check Constitution compliance.**

---

**END OF CONSTITUTION**

*This document is binding. When in doubt, choose the path that maximizes determinism, observability, and canon fidelity.*
