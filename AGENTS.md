...
- Semantic UGC must reclaim only expired empty candidate reservations, reuse checksum-addressed prior paid takes for QA-only repair without a provider submission, and allow the bounded 0.48s sentence pause needed for natural 16s acoustic delivery before requesting another paid retry.
