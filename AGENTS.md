...
- Semantic UGC actor identity comes exclusively from the two ordered actor reference images; never require, snapshot, or inject a written character description into shot-frame or video generation. Treat legacy anchors as unverified: a usable scene plate must carry the current generation-contract hash, a server-computed original-reference identity gate with every component true, and an attributable human attestation. Re-download and checksum both originals before any paid video request, then require independent final actor-identity and scene-continuity gates against the delivered contact sheet.
...
- Semantic UGC seam and acoustic continuity gates apply only to multi-take delivery; persist one-take gates as `not_applicable`, sort ffprobe frame evidence before deduplication, normalize bounded codec preroll to time zero, and reject materially negative or non-finite timestamps. Localize an unsatisfied post-word guard to the predecessor rather than blindly retrying the incoming take; never manufacture an internal acoustic margin by cloning talking-head video frames, and conservatively exclude the final 350 ms of every non-final provider take unless deterministic frame analysis locates a later whole-frame terminal reset. A measured reset may use only a bounded 100 ms audio L-cut with at most one frame of spoken overhang, must cut picture before the first bad frame, and must borrow clean incoming visual preroll so delivered A/V duration remains exact; otherwise force a localized retry. Use Semantic-only intentional jump-cut reframes while preserving legacy full framing by default, then require delivered-boundary frozen-frame QA before captions or upload. When clean incoming preroll can deterministically bridge a safe predecessor, keep the analysis boundary inside AAC frame quantization, and map a validated final transcript’s seams from canonical spoken-word counts rather than typography-style beat counts.
- Semantic UGC duration recovery must prefer complete source sentences, reject internal excerpt/source-review language at the persistence boundary, and use a deterministic audience-safe, contract-valid fallback after bounded rewrite attempts; do not abort batch seeding solely because the model keeps returning structurally invalid copy.
- Semantic UGC local startup must route text generation through a provider that passes a real generate-content probe; when local Vertex lacks `aiplatform.endpoints.predict` and the configured Gemini API key succeeds, set `GEMINI_PROVIDER=gemini_api` with `GEMINI_API_FALLBACK_ENABLED=true` before resuming stuck S1 batches.
- Vertex video submission must verify `VERTEX_REQUIRED_PRINCIPAL_EMAIL` against the live ADC principal and fail closed before any provider request when the identity is missing, unverifiable, or different.
- Semantic scene and final-video identity QA must use a current Vertex `generateContent` model with image-input support in the configured region; verify every model-ID default against Google's official catalog and a real endpoint probe instead of inventing versioned model names.
