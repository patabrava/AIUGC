...
- VEO `referenceImages` for Character Consistency are reserved exclusively for ActorIdentity anchor images selected from the persisted actor identity image set. Approved Magnific `SceneReferenceImage` stills are review artifacts and scene/wardrobe approval metadata only; they must not be submitted as VEO visual references because any face/person in a scene still can compete with the actor identity.
- The submitted VEO prompt must describe the approved scene in text from the scene bible and must state that reference images are only the actor identity source, not the room, wardrobe, or layout source.
- If a route cannot submit three actor identity anchors as VEO `referenceImages`, the route must block with a visible validation error instead of silently generating a non-anchored video.
