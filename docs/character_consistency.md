...
- The approved three-image `SceneReferenceImage` set is the LoRA-to-VEO bridge. VEO 3.1 cannot consume Magnific LoRA weights directly, so the app must pass the three approved LoRA-generated stills as `referenceImages` on every 8-second-compatible Character Consistency base request.
- Scene reference images are not approval-only artifacts. They become paid-provider visual inputs after the operator confirms actor identity match and scene consistency for the full set.
- Raw ActorIdentity training images are not a substitute for approved LoRA-generated scene stills in ActorIdentity-backed Character Consistency batches.
- If a route cannot submit the approved LoRA-generated stills as VEO `referenceImages`, the route must block with a visible validation error instead of silently generating a non-LoRA-backed video.
